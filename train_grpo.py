...
SCENARIO = os.getenv("SCENARIO", "01_baseline")
...
def env_reset(difficulty="easy", scenario="01_baseline"):
...
def compute_episode_reward(model, tokenizer, difficulty="easy", scenario="01_baseline", max_steps=50):
    """
    ...
    obs = env_reset(difficulty, scenario)
...
def train():
    ...
    print(f"[3/4] Running {NUM_EPISODES} training episodes on '{DIFFICULTY}' difficulty, '{SCENARIO}' scenario...")
    ...
    for ep in range(NUM_EPISODES):
        reward, data = compute_episode_reward(model, tokenizer, DIFFICULTY, SCENARIO)
