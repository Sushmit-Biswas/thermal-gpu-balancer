"""
Quick manual test for the ClusterOps environment server.

Usage:
    1. Start the server: uvicorn server.app:app --port 8000
    2. Run: python test_env.py
"""
...
# Reset
r = requests.post(f"{ENV_URL}/reset", json={"difficulty": "medium"})
obs = r.json()
...
if queue:
    ...
    r = requests.post(f"{ENV_URL}/step", json={
        "action_type": "allocate", "job_id": job["id"], "node_id": 0
    })
    d = r.json()
...
# Wait 6 steps
for i in range(6):
    r = requests.post(f"{ENV_URL}/step", json={"action_type": "wait"})
    d = r.json()
    od = d["observation"]
